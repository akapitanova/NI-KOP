...
inline void best_new_max (best_list_t list, unsigned* occ, int v) {
    list[*occ] = v;
    *occ = *occ + 1;
}
inline void best_new (best_list_t list, unsigned* occ, int v) {
    *occ = 1;
    list[0] = v;
}
/*-----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------*/
double func_f (inst_t* inst, cnt_t cnt, int cli, int var, var_info_t varinf, sol_t sol, int satisfied) {
    int gain = sol[var] > 0 ? gw_neg_flip_gain (varinf, cnt, var) : gw_pos_flip_gain (varinf, cnt, var);

    int not_satisfied = inst->length - (satisfied + gain);
    //printf("Var: %d, gain: %d, nots: %d", var, gain, not_satisfied);
    return ((double) 1.0) / pow((EPS + not_satisfied),((double) 2.3));
}  

int gw_pick_var_proport_to_f (inst_t* inst, cnt_t cnt, int cli, var_info_t varinf, sol_t sol, int satisfied) {
    literal_t* clause;
    int i;
    double * probs = (double*) malloc(sizeof(double) * (inst->width + 1));
    double sumProb = 0.0;
    ...
    for (i=0; i<inst->width; i++) {
      ...
      probs[i] = func_f(inst, cnt, cli, clause[i] < 0? -clause[i]: clause[i], varinf, sol, satisfied);
      sumProb += probs[i];
    }
    randomPosition = rng_next_double() * sumProb;
    //for (int j = i-1; j >= 0; j--) {
    //  printf("j: %d: %f ",j, probs[j]);
    //}
    //printf("\n");
    for (i = i-1; i != 0; i--) {
      sumProb -= probs[i];
      if(sumProb <= randomPosition) break;
    }
    //printf("%d ", i);
    free(probs);
    if (clause[i] < 0) return -clause[i];
    return clause[i];
}
...
/*  inst_write(&inst, stderr); */
    ...
    while (satisfied < inst.length && cont && ((!triesmax) || tryno <= triesmax)) {
        ...
        flipvar = gw_pick_var_proport_to_f  (&inst, cnt, ucli, varinf, sol, satisfied);               /* pick a variable in that clause */
